...
## 2026-07-14 Logic Topo 链路表结构推断布局

本轮补充无环链识别表时的 Logic Topo 布局兜底能力，降低逻辑视图对外部环链结果的强依赖。

- 上传环链表时仍优先使用 `Member_path` 作为环/链布局种子。
- 未上传环链表或没有有效 `Member_path` 时，Logic Topo 会基于当前网元表和链路表自动推断布局路径。
- 推断逻辑会扫描 CSG-CSG 连通分量，并识别这些 CSG 分量相邻的 ASG 上联节点。
- 每个 CSG 分量会形成布局用的 ASG-CSG-ASG 或 ASG-CSG 路径，用于初始布局和虚拟布局边。
- 推断路径只服务于 Logic Topo 布局，不会写回上传数据，不会生成环链表记录，也不影响环链字段过滤统计。
- 布局缓存版本升级为 `logic-v3-inferred-structure`，避免沿用旧版环链优先布局缓存。
