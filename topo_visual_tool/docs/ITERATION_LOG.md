...
## 2026-07-09 GIS 路网路径持久化图层

本轮按用户确认的“路网路径坚持持久化显示”方案，将路网路径从普通一次性 Canvas overlay 改为 Leaflet 自定义 Canvas Layer。

- 新增 `L.RouteCanvasLayer`，路网路径 Canvas 挂载到 Leaflet `overlayPane`，移动和缩放过程中不再在 `movestart` / `zoomstart` 清除。
- 路网图层在地图移动、缩放结束后按当前投影重绘，保持路径位置准确，同时避免拖拽和缩放期间频繁消失又重现。
- `renderMap()` 不再每次销毁 routes 图层，而是复用持久化 route layer 并通过 `setData()` 更新路由数据。
- 当用户关闭路网路径显示、上传新数据或当前视图没有有效 `Route WKT` 时，统一通过 `clearRouteLayers()` 移除持久化图层和命中缓存。
- 大数据链路裁剪从“源宿任一端点在视窗内”调整为“链路包围盒与扩展视窗相交”，减少长跨度链路放大后消失的问题。
- 已通过 Edge headless 自动化验证：上传 `thailand_colocated_large_device.csv` 和 `thailand_colocated_large_link.csv` 后，1160 条路由全部进入路由命中缓存；`movestart`、`zoomstart`、`moveend`、`zoomend` 过程中 route Canvas 始终保持 1 个持久图层，并且路由点击命中返回对应 link key。
