...
将 `logic_component_300_*` 从单一 ASG 对极限压力样例调整为更符合运营商城域接入网的多汇聚片区样例，并继续用于验证环链组件化布局在 300 节点规模下的可读性。

### 实现内容

- 重写 `scripts/generate_logic_component_sample.js`，生成 4 个城域汇聚片区、4 台 PE、8 台 ASG、28 个接入环和 FTTx 支线接入点。
- 新样例保持 300 网元规模，链路表包含 353 条链路，覆盖核心互联、PE-ASG 上联、接入环、相邻环保护、跨汇聚保护和支线链路。
- 环链表仅保留 28 条 `Category=Ring` 接入环记录，保护链路保留在链路表中，避免被误拆成额外环链组件。
- 网元表新增 `Site Type` 字段，便于按 Core POP、Aggregation Hub、Mobile Backhaul、Business Access、FTTx Spur 做样式或过滤验证。
- Logic Topo 节点重叠消解补充近似垂直贴近节点的确定性横向分离，并将组件化布局的重叠消解轮次提升到 10 轮。
- 布局验证脚本新增最近冲突节点对输出，便于定位布局退化点。

### 测试记录

- 执行 `node scripts/generate_logic_component_sample.js`，生成 300 网元、353 链路、28 环链记录的真实化接入网样例。
- 执行 `node --check assets/js/app.js`，业务脚本语法检查通过。
- 执行 `node scripts/verify_logic_component_layout.js`，300 节点样例最小节点中心距离为 32.67px，`<24px` 节点距离冲突为 0，`<38px` 节点距离冲突为 3，标签盒重叠为 98 对。

## 2026-07-24 Logic Topo 环链组件化布局

### 目标

优化上百节点 Logic Topo 中环链结构混杂的问题，让同一 ASG 对下挂的多个接入环保持可识别的业务组件形态。

### 实现内容

- Logic Topo 在存在环链路径时优先启用环链组件化布局，不再把所有节点统一投入全局 Kamada-Kawai / spring 优化。
- 环链记录按 `Belong_agg`、`Uplink_pair`、`Root1***Root2` 和 `Name` 聚合为组件，每个组件按路径数量和成员数量分配独立布局盒。
- 组件内部将共享 ASG 根节点固定为顶部锚点，`Category=Ring` 路径按 `Member_path` 顺序放入独立椭圆 cell，`Category=Link` 路径放入链式 lane。
- 组件内未进入 `Member_path` 但与组件相连的节点作为 peripheral 节点放在组件边缘。
- Logic 节点标签新增按布局点指定方向的能力，环上标签向外扩散、链上标签上下错开、根节点标签置于锚点外侧。
- 新增 `logic_component_300_*` 测试数据生成脚本，构造一对 ASG 下挂 24 个接入环、少量环间连接和 spur 节点的 300 节点样本。
- 新增布局验证脚本，直接调用前端布局函数统计节点距离和标签盒重叠。

### 测试记录

- 执行 `node --check assets/js/app.js`，业务脚本语法检查通过。
- 执行 `node scripts/generate_logic_component_sample.js`，生成 300 网元、331 链路、32 环链记录的测试数据。
- 执行 `node scripts/verify_logic_component_layout.js`，300 节点样本的最小节点中心距离为 63.98px，`<24px` 和 `<38px` 节点距离冲突均为 0，标签盒重叠为 60 对。

## 2026-07-24 GIS 瓦片加载与灰网格优化

### 目标

减少刷新页面、拖拽和缩放 GIS 地图时在线瓦片加载空窗导致的灰色网格或零散瓦片长时间残留问题。

### 实现内容

- 主拓扑 GIS 和版本对比 GIS 统一复用在线瓦片创建与健康检查逻辑。
- 在线瓦片改为 `updateWhenIdle=true`、`updateWhenZooming=false`、`updateInterval=280`、`keepBuffer=8`，降低拖拽/缩放过程中的请求抖动并保留更多旧瓦片缓冲。
- 新增瓦片 pending 计数和慢加载超时检测，瓦片长时间未完成加载时主动切换本地浅色简洁底图。
- 将连续瓦片错误 fallback 阈值从 24 降低到 8，缩短不稳定网络下的灰底暴露时间。
- 地图容器和对比页地图容器取消可见网格背景，改为纯浅色兜底背景。

### 测试记录

- 执行 `node --check assets/js/app.js`，业务脚本语法检查通过。

## 2026-07-21 样式规则条件来源拓展

### 目标

补齐节点样式规则和链路样式规则的条件来源能力，使样式筛选可以像过滤、高亮、定位一样使用网元表、链路表和环链表字段。

### 实现内容

- 节点样式规则和链路样式规则的复合条件弹窗新增“条件来源”选择，复用现有条件组模型 `source`、`mode`、`conditions`。
- 节点样式规则支持基于网元字段、链路字段或环链字段筛选命中网元；链路字段命中后取链路源宿网元，环链字段命中后取 `Member_path` 中有效成员网元。
- 链路样式规则支持基于网元字段、链路字段或环链字段筛选命中链路；网元字段要求链路两端均命中，环链字段只作用于命中环链记录 `Member_path` 的相邻路径段。
- 样式模板导入、字段修剪和对比页样式规则同步保留每条规则的 `source`；旧模板缺省时仍按节点规则 `nodes`、链路规则 `links` 兼容。
- 主拓扑样式匹配新增规则命中缓存，避免绘制节点和链路时重复扫描链路表、环链表。

### 测试记录

- 执行 `node --check assets/js/app.js`，业务脚本语法检查通过。
- 使用 Node VM 验证环链字段节点样式规则命中 `Member_path` 成员，环链字段链路样式规则命中相邻路径段，网元字段链路样式规则仅命中两端均满足条件的链路。
- 启动本地静态服务并访问 `http://127.0.0.1:8000/topo_visual_tool.html`，页面返回 200。
...
本轮新增“版本拓扑对比”能力，用于在同一页面内横向查看两个数据版本的局部 GIS 拓扑差异。

- 顶部页签新增“版本拓扑对比”入口，与“拓扑视图”“数据管理”并列。
- 对比页提供左、右两个版本选择框，数据直接读取当前会话中的 `state.versions`。
- 左右两侧分别创建独立 Leaflet 地图实例，支持拖拽、缩放、点击网元/链路查看详情。
- 支持“同步视野”开关；开启时一侧地图移动或缩放会同步另一侧地图中心点和缩放级别。
- 支持按中心网元 1/2/3 Hop 展开局部拓扑，左右版本分别按自身链路关系计算邻域。
- 支持粘贴网元清单或链路清单进行局部对比；清除条件后恢复显示所选版本全部拓扑。
- 每个 GIS 窗口左侧新增默认隐藏的样式抽屉，可控制路网路径显示、差异高亮显示和链路线宽。
- 新增基础差异高亮：左侧独有对象红色、右侧独有对象绿色、同名但属性可能变化的网元黄色描边。
- 第一版仅实现 GIS 对比；Logic Topo 双窗口、地图框选、字段级差异面板和结果导出留作后续迭代。
- 同步更新 `DESIGN.md` 和 `README.md`；后续涉及功能或交互改动时，需同步维护设计文档、迭代日志和 README。

## 2026-07-16 版本拓扑对比样式抽屉完善

本轮修正对比页第一版交互问题，并补齐样式设置能力。

- 对比页每侧样式抽屉新增角色图例样式编辑，支持按 `CSG`、`ASG`、`PE`、`OTHER` 配置颜色、大小和形状。
- 对比页每侧样式抽屉新增节点样式规则，支持按当前版本网元字段设置条件、颜色、形状和大小。
- 对比页每侧样式抽屉新增链路样式规则，支持按当前版本链路字段设置条件、颜色、线型和线宽。
- 对比页样式规则采用临时视图状态，修改后立即重绘当前侧地图，不写回主拓扑页全局样式或数据版本。
- 对比页 Leaflet 缩放控件从默认左上角移动到右下角，避免遮挡左侧滑出的样式抽屉和“显示路网路径”控件。
- 对比详情浮层上移，为右下角缩放控件预留空间。

## 2026-07-16 对比页样式规则复合条件与导入

本轮继续完善版本拓扑对比页的样式规则适配能力。

- 对比页节点样式规则和链路样式规则改为复用主拓扑视图的规则模型，保留 `conditions` 与 `mode`，不再只支持单字段条件。
- 对比页规则行新增条件摘要和“复合条件”按钮，复用现有复合条件弹窗编辑多字段条件。
- 复合条件弹窗针对对比页左右两侧分别读取所选版本的网元字段或链路字段，并提供对应字段值候选。
- 对比页每侧样式抽屉新增“导入样式配置”入口，兼容主拓扑视图导出的样式配置 JSON。
- 导入样式配置时，角色图例样式、节点样式规则和链路样式规则会更新到当前侧对比窗口；规则字段按当前侧所选版本字段清洗，不存在的字段会回退到可用字段并清空条件值。
- 对比页导入的样式仍为临时视图状态，不写回主拓扑页全局样式或数据版本。

## 2026-07-17 对比页过滤与高亮条件

本轮补齐版本拓扑对比页中每侧 GIS 窗口的过滤和高亮能力。

- 对比页每侧样式抽屉新增“高亮条件”和“过滤条件”分组。
- 高亮/过滤均复用现有复合条件弹窗，支持“全部满足 / 任一满足”的多字段组合。
- 对比页复合条件弹窗会按当前侧所选版本读取网元字段、链路字段和环链字段，并提供对应字段值候选。
- 过滤条件只作用于当前侧 GIS：按网元条件保留命中网元及内部链路，按链路条件保留命中链路及端点，按环链条件保留命中环链成员及内部链路。
- 高亮条件只作用于当前侧 GIS：命中对象保持强调，未命中网元和链路降低透明度。
- 清除高亮或过滤后，当前侧地图立即恢复对应状态，不影响另一侧版本窗口。

## 2026-07-17 过滤与高亮后自动局部聚焦

本轮优化过滤/高亮后的 GIS 查看体验，减少用户手动缩放查找命中对象的操作。

- 主拓扑视图中，点击“应用过滤”后自动将 GIS 地图聚焦到过滤后的可见网元范围。
- 主拓扑视图中，点击“应用高亮”后自动将 GIS 地图聚焦到高亮命中的网元和命中链路端点范围。
- 通过复合条件弹窗应用过滤/高亮时，同样触发自动局部聚焦。
- 版本拓扑对比页中，每侧过滤/高亮应用后只聚焦当前侧 GIS 窗口，不直接改变另一侧的条件状态；若开启同步视野，Leaflet 视野同步逻辑仍会同步地图中心和缩放。
- 当命中对象没有有效经纬度时，不强制改变当前视野。

## 2026-08-03 网元定位单点强聚焦

本轮优化“网元定位”在命中单个网元时的聚焦尺度，解决地图跳转后周边节点仍过多、目标不容易被发现的问题。

- GIS Topo 单网元定位从普通局部缩放提升为高缩放级别强聚焦，目标网元尽量位于视图中心并保留极少量上下文。
- Logic Topo 单网元定位时先将缩放提升到最高交互缩放级别，再居中目标节点。
- 多网元定位和复合条件批量定位仍保留原有范围适配策略，避免过度放大导致命中集合不可见。
