...
## 2026-07-09 链路字段高亮/过滤条件来源

本轮将高亮条件和过滤条件从网元/环链来源扩展为支持链路表字段来源。

- 复合条件弹窗的条件来源新增“链路字段”，与“网元字段”“环链字段”保持同一交互模式。
- 链路字段条件先匹配 link 表记录，再用命中链路的 `Src NE Name` 和 `Sink NE Name` 形成命中网元集合。
- 链路字段过滤只保留命中链路表条件的链路及其源宿网元，不会额外带入同一批端点之间的其他未命中链路。
- 链路字段高亮只让命中链路及其源宿网元保持原样式，其余元素继续按用户设置的对比度淡化。
- 链路字段条件值输入框复用当前 link 表枚举值和历史输入值联想。
- README 和 DESIGN.md 已同步记录三类条件来源及链路字段来源的过滤/高亮语义。

## 2026-07-09 GIS 直连链路视窗裁剪优化

本轮按用户要求忽略路网路径渲染相关功能，仅优化高亮/过滤来源确认和 GIS 直连链路显示稳定性。

- 确认高亮/过滤复合条件已支持网元字段、链路字段和环链字段三类来源。
- 保留现有 GIS 直连链路 `L.polyline` 渲染方式，不再改为 Canvas 合批，避免引入拖拽残影。
- 大数据模式下，直连链路裁剪从“源宿任一端点在视窗内”改为“链路经纬度包围盒与扩展视窗相交”。
- 长跨度链路只要穿过当前扩展视窗，即使源宿端点都在视窗外，也会被纳入本轮地图渲染候选，减少放大后链路突然消失的问题。
- Leaflet Canvas renderer padding 从 `0.35` 提升到 `0.75`，降低视窗边缘链路在缩放/拖动后的闪现概率。

## 2026-07-10 复合条件批量定位

本轮将网元定位从单一网元名称搜索扩展为复合条件批量定位。

- 网元定位卡片保留原有快速输入框，并新增“复合条件”和“清除”按钮。
- 快速定位默认按 `网元表 / NE Name / 包含` 执行，继续支持网元名称联想和历史输入。
- 复合定位复用高亮/过滤条件弹窗，支持网元字段、链路字段和环链字段三类条件来源。
- 定位规则作用于当前可视拓扑范围；当过滤或批量查询已生效时，不会定位到当前已被过滤掉的网元。
- 网元字段和环链字段来源先解析命中网元，再加粗源宿都在命中集合内的内部链路。
- 链路字段来源先匹配链路表记录，再定位命中链路的源宿网元并加粗这些命中链路。
- GIS 视图按命中网元有效经纬度执行单点 `setView` 或多点 `fitBounds`；Logic Topo 按命中网元布局包围盒调整 pan/zoom。
- 未命中时在定位卡片显示“未定位到符合条件网元”，缺少有效经纬度时显示 warning，不中断 Logic Topo 定位态。
- 定位态独立于高亮和过滤规则，可单独清除；命中网元和链路使用加粗描边/线宽表达，不覆盖用户已有颜色样式。

## 2026-07-10 样式配置模板导入导出

本轮新增样式配置模板能力，解决刷新 HTML 或替换新版 HTML 后需要重新配置节点/链路样式的问题。

- 在“样式设置”卡片内、样式规则上方新增“保存当前样式设置”和“上传样式配置文件”入口。
- 导出的 JSON 使用 `schema=topo_visual_tool_style_template`、`version=1`，包含角色样式、节点样式规则、链路样式规则、环链样式规则和路网路径样式。
- 上传配置文件后立即覆盖当前样式配置并应用，无需再次点击“应用节点规则”或“应用链路规则”。
- 导入时会校验 schema 和版本；JSON 无效、schema 不匹配或版本过高时阻止导入并提示错误。
- 导入时会清洗颜色、形状、线型、线宽和透明度等字段；当前数据不存在的规则字段会回退到可用字段并清空条件值。
- 空规则或异常规则会被忽略，并在样式模板消息区提示调整和忽略数量。
- README 和 DESIGN.md 已同步记录模板格式、导入应用策略和异常处理规则。

## 2026-07-14 Logic Topo 环链优先布局优化

本轮根据 `topo_layout_algo.md` 的布局算法思路，优化 Logic Topo 的默认布局策略。

- Logic Topo 布局优先按照环链结构呈现，不再强制按 PE/ASG/CSG 角色层级吸附。
- 上传环链表后，`Category=Ring` 的 `Member_path` 会作为环形/椭圆初始布局，`Category=Link` 的 `Member_path` 会作为链式初始布局。
- 环链 `Member_path` 相邻节点会作为虚拟布局边参与力导约束，帮助保持环、链路径连续性。
- 未被环链覆盖的节点按当前可见链路的连通分量进行初始布局，再与环链节点共同进入优化。
- 小规模图使用浏览器端 Kamada-Kawai-like 全局距离优化，中规模图使用增强 spring layout。
- spring layout 移除角色层级目标 Y 约束，改为围绕环链/组件 seed 和真实链路关系进行收敛。
...
## 2026-07-14 Logic Topo 虚拟画布布局优化

本轮针对多个环链被压缩在当前屏幕矩形内的问题，解耦 Logic Topo 的布局坐标空间和可见 SVG 视口。

- 新增 `state.logic.layoutWidth` 和 `state.logic.layoutHeight`，布局算法在虚拟逻辑画布内计算坐标。
- 虚拟画布尺寸根据当前节点数量、环链数量和视口尺寸自适应扩大，避免中小规模环链全部挤在当前屏幕尺寸内。
- `normalizeLogicPositions()` 改为归一到虚拟画布，不再强制放大/压缩到可见 viewport。
- 初次布局和“适配视图”通过 pan/zoom 将虚拟布局 fit 到当前视口，而不是改变节点原始布局坐标。
- 批量定位和单点定位继续基于虚拟坐标计算聚焦区域。
- 环链初始布局从逐条 `Ring/Link` 记录网格切分，改为按 `Belong_agg` / `Uplink_pair` 聚合为业务组件。
- 每个业务组件内主环保留最小视觉半径，环上链放置在环内或环侧，避免链记录抢占独立大格子导致主环过小。
## 2026-07-15 同 HTML 多版本数据切换

本轮将项目名称从单一页面元数据扩展为数据版本管理入口，支持同一个 HTML 页面会话中创建、上传、切换多份拓扑数据。

- 顶部工具栏新增“数据版本”下拉框、“新建版本”和“删除版本”按钮。
- 当前“项目名称”输入框会同步重命名当前数据版本。
- 上传解析网元表、链路表和可选环链表时，解析结果只写入当前激活版本。
- 版本切换会恢复对应版本的网元、链路、环链、索引、质量统计、过滤条件、高亮条件、定位条件和批量查询状态。
- 节点/链路/环链样式规则、路网路径样式、语言和历史条件继续作为全局配置共享。
- 删除最后一个版本时保留一个空版本，避免页面进入无激活数据版本状态。
- 新增 `version_alpha_device.csv` / `version_alpha_link.csv` 和 `version_beta_device.csv` / `version_beta_link.csv`，用于验证同页面多版本上传和切换。
## 2026-07-15 多版本上传来源回显

本轮优化多版本数据切换下的上传卡片交互，避免切换版本后用户无法判断当前版本的数据来源。

- 上传区三类文件输入改为“选择文件”按钮加来源显示条的紧凑布局。
- 每个数据版本新增 `sourceFiles` 元数据，记录网元表、链路表和可选环链表的来源文件信息。
- 用户选择文件后立即更新当前版本来源显示；解析成功后随版本一起保存。
- 切换版本时恢复该版本对应的文件来源显示，不再让侧边栏只显示“未选择文件”。
- 受浏览器安全限制，页面不能程序化回填原生 file input，也通常不能读取真实绝对路径；当只能获取文件名时，tooltip 会提示仅记录文件名。
- Mock 数据会显示为“内置 Mock 数据”来源。
