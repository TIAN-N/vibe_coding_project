...
## 2026-07-09 GIS 图层稳定渲染优化

本轮优化 GIS 拖拽/缩放时路网路径和直连链路的稳定性，并降低大量链路场景下的 Leaflet 图层开销。

- 地图 `movestart` / `zoomstart` 时不再清空路网路径 Canvas，移动期间保留现有图层，停止后再按当前视口重绘。
- GIS 直连链路从逐条 `L.polyline` 改为单 Canvas 合批绘制，白色底线和业务色线在同一 Canvas pass 中绘制，选中链路二次置顶绘制。
- 新增 `linkHitEntries` 和 `findLinkHit()`，通过像素级线段距离保留 GIS 直连链路点击详情能力。
- 大数据视窗裁剪从“源宿任一端点在视窗内”改为“源宿线段包围盒与扩展视窗相交”，减少长跨度链路放大后突然消失的问题。
- Leaflet renderer padding 从 `0.35` 提升到 `1.2`，瓦片层改为 idle 后更新并扩大 keep buffer，降低缩放拖动期间的视觉闪动。
- 已通过 Edge headless 页面自动化验证：加载 `device.csv` / `link.csv` 后，GIS 生成 1 个链路 Canvas 和 1 个路网 Canvas；移动期间两者保持可见；移动结束后重绘稳定；直连链路 hit-test 能命中并返回对应 link key。
