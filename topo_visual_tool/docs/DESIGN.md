...
- 路网路径使用 Leaflet 自定义 Canvas Layer 持久化显示；地图开始平移或缩放时不移除当前 Canvas，图层跟随 Leaflet pane 生命周期保持可见，地图停止后再按新视口重绘。
...
`Route WKT` paths are rendered only in GIS view. They use one persistent Leaflet custom Canvas layer rather than one Leaflet layer per route. Canvas paths support visibility, color, width, line style, and opacity configuration. During map movement or zooming, the existing route Canvas remains attached to the Leaflet overlay pane so routes stay visible; after the map stabilizes, the layer redraws using the current projection.
