...
- 地图开始平移或缩放时，不再立即移除当前路网 Canvas 图层；旧图层跟随 Leaflet pane transform 保持可见，地图停止后再按新视口重绘，避免用户看到路网路径频繁消失和重现。
...
`Route WKT` paths are rendered only in GIS view. They use a Canvas overlay rather than one Leaflet layer per route. Canvas paths support visibility, color, width, line style, and opacity configuration. During map movement or zooming, the existing route Canvas remains visible with the Leaflet pane transform, then it is recreated after the map stabilizes. This favors visual continuity over clearing the layer at movement start.
...
### 14.5.1 GIS Link Rendering Contract

GIS direct links are rendered through one batched Canvas overlay rather than two Leaflet polylines per link. The white outline and business-colored line are drawn in the same Canvas pass, while selected links are drawn again on top to preserve visual priority.

The link Canvas keeps a `linkHitEntries` cache for pixel-distance hit testing. Clicking a direct link still opens the link detail card. In large datasets, link viewport clipping uses source/sink bounds intersection with the padded map bounds rather than only checking whether either endpoint is inside the viewport, so long links crossing the viewport remain visible after zooming in.
