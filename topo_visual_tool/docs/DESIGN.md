...
后续涉及功能、交互、数据契约、性能策略或页面结构的迭代改动，都需要同步更新：

- `docs/DESIGN.md`：记录需求分析、设计方案、核心边界和技术取舍。
- `docs/ITERATION_LOG.md`：记录本轮实际完成内容、行为变化和验证结果。
- `docs/README.md`：面向使用者更新入口、操作方式和能力清单。
...
新增“版本拓扑对比”页签，用于在同一页面内横向比较两个数据版本的局部 GIS 拓扑。

设计目标：

- 用户已经通过“数据版本”管理加载两份或多份不同数据。
- 对比页左、右两侧各选择一个版本，并分别实例化独立 Leaflet GIS 地图。
- 两侧地图支持缩放、拖拽、点击网元/链路查看详情、适配当前范围。
- 支持同步视野开关：开启后，一侧地图拖拽或缩放会同步另一侧中心点和缩放级别。
- 支持局部对比：
  - 输入中心网元并选择 1/2/3 Hop，左右版本分别按邻接关系展开局部拓扑。
  - 粘贴网元清单或链路清单，左右版本分别匹配并显示相关局部拓扑。
  - 清空条件后显示所选版本全部拓扑。
- 每侧 GIS 窗口左上方提供“样式设置”抽屉，默认隐藏，点击按钮滑出。第一版提供路网路径显示、差异高亮显示和链路线宽调整。
- 样式设置抽屉需要覆盖主拓扑视图中常用的样式能力：角色图例样式、节点样式规则、链路样式规则。对比页内的编辑采用“临时视图样式”，只影响当前侧 GIS 对比窗口。
- 对比页节点/链路样式规则复用主拓扑视图的规则数据模型：`source`、`mode`、`conditions`、`field`、`op`、`value` 和视觉字段保持一致，并通过同一个复合条件弹窗编辑多字段条件。
- 对比页样式抽屉支持导入主拓扑视图导出的样式配置 JSON；导入时按当前侧所选版本的网元/链路字段清洗规则，字段不存在时回退到该版本可用字段并清空条件值。
- 对比页样式抽屉同步提供主拓扑视图中的过滤条件和高亮条件功能。过滤/高亮均使用同一套复合条件弹窗，可按当前侧版本的网元、链路或环链字段筛选；过滤只改变当前侧 GIS 可见对象，高亮只在当前侧 GIS 中强调命中对象并淡化未命中对象。
- 主拓扑视图和对比页的过滤/高亮条件在应用后会自动执行 GIS 局部聚焦：过滤聚焦过滤后的可见网元范围，高亮聚焦命中网元和命中链路端点范围；没有有效坐标命中时保持当前视野不变。
- 对比页 Leaflet 缩放控件放置在右下角，避免与左侧滑出样式抽屉重叠。
- 差异高亮规则：
  - 左侧存在、右侧不存在的网元/链路使用红色强调。
  - 右侧存在、左侧不存在的网元/链路使用绿色强调。
  - 两侧同名网元存在但属性 JSON 不一致时使用黄色描边提示可能变化。

实现边界：

- 第一版只实现 GIS 双窗口对比，不实现 Logic Topo 双窗口。
- 对比页使用独立地图上下文，避免复用主页面单一 `state.map` 造成互相覆盖。
- 对比页样式为临时视图设置，不写回具体数据版本；角色颜色和基础节点样式继续沿用当前全局样式配置。
- 地图框选、字段级差异报告、导出对比结果和样式保存回版本留作后续迭代。
